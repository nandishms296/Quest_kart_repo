         ...
         uploadFileToGitHub(
           ...
           .catch((err) => {
             ...
             res.status(500).send({
               message:
                 err.message ||
                 "Some error occurred while creating the pipeline.",
             });
           });
       })
       .catch((err) => {
         logger.error(
           "error occurred while creating the pipeline",
           { exportFunction: "pipeline.controller.create" }
         );
         res.status(500).send({
           message:
...
exports.findAll = (req, res) => {
    ...
    db.sequelizeConfig.query(
            ...
            {model: Pipeline})
    .then(data => {
        logger.info("retrieving all the pipelines",{exportFunction: "pipeline.controller.findAll"});
        res.send(data);
    })
    .catch(err => {
        logger.error("error occurred while retrieving the pipeline",{exportFunction: "pipeline.controller.findAll"});
        res.status(500).send({
            message:
...
exports.findAllById = (req, res) => {
    ...
    db.sequelizeConfig.query(
            ...
            {model: Pipeline})
    .then(data => {
        logger.info("retrieving the pipelines by id",{exportFunction: "pipeline.controller.findAllById"});
        res.send(data);
    })
    .catch(err => {
        logger.error("error occurred while retrieving pipeline by id",{exportFunction: "pipeline.controller.findAllById"});
        res.status(500).send({
            message:
            err.message || "Some error occurred while retrieving Pipeline."
        });
...
exports.findOne = (req, res) => {
    ...
    .then(data => {
        if (data) {
            logger.info("finding the pipeline by id",{exportFunction: "pipeline.controller.findOne"})
        res.send(data);
        } else {
...
exports.findAllActive = (req, res) => {
    ...
    .catch(err => {
      ...
      res.status(500).send({
          message:
          err.message || "Some error occurred while retrieving active Pipeline."
      });
    }); 
};
...
exports.getIdAndName = (req, res) => {
    ...
    let sql = "SELECT id, pipeline_name as name FROM tbl_pipeline where is_active = 'Y'";
    console.log("SQL: ", sql);
